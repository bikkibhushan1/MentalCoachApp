...
export default class FirebaseSupport {
  static registerForPushCallback(log, client) {
    log.info(
      'FirebaseSupportModule.JS.registerForPushCallback',
    ...
    NativeModules.FirebaseSupportModule.getFcmToken()
      .then(fcmToken => {
        ...
        updatePinpointEndpoint(fcmToken, 'pinpoint-fcm');
      })
      .catch(err => {
        log.error(
          'FirebaseSupportModule.JS.registerForPushCallback',
          'error while requesting FCM token',
          err,
        );
      });
  ...
  static showPushCallback(log, push) {
    log.info(
      'FirebaseSupportModule.JS.showPushCallback',
      'show notification',
      push,
    );
